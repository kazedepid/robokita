module.exports = {
  ...
  func: async (conn, m, text) => {
    ...
    if (!text || !text.startsWith('+')) return await m.reply({ text: '*Please enter the number of user with +countrycode for rejecting!*' });
    let list = [];
    ...
    await conn.groupRequestParticipantsUpdate(m.chat, [text.replace('+', '') + '@s.whatsapp.net'], 'reject');
    return await m.reply({ text: '*Rejected @' + text.replace('+', '') + '\'s join request!*' });
  }
};
